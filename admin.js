  ...
  $("ideasList").innerHTML = ideas.map(item => {
    ...
    return `
      <article class="idea-card">
        ...
        ${item.attachment_path ? `<div class="idea-attachment"><div><b>📎 الملف المرفق</b><span>${escapeHtml(item.attachment_name || "ملف مرفق")}</span></div><button type="button" onclick="downloadAttachment(${item.id}, '${escapeJs(item.attachment_name || "attachment")}')">تحميل الملف والاطلاع عليه</button></div>` : ""}
...
async function downloadAttachment(id, fileName) {
  try {
    const res = await fetch(`/api/admin/ideas/${id}/attachment`, {
      headers: { "x-admin-username": username, "x-admin-password": password }
    });
    if (!res.ok) {
      const data = await res.json().catch(() => ({}));
